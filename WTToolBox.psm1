...
if ($IsWindows -OR $PSEdition -eq 'Desktop')
{
    ...
    Write-Verbose "Testing for Microsoft.WindowsTerminalPreview"
    $app = Get-AppxPackage Microsoft.WindowsTerminalPreview
    if ($app)
    {
        Write-Verbose "Windows Terminal is installed"
        Write-Verbose "Testing for settings.json"
        if (Test-Path -Path "$ENV:Userprofile\AppData\Local\Packages\Microsoft.WindowsTerminalPreview_8wekyb3d8bbwe\LocalState\settings.json")
        {
            #Export settings path to a global variable
            Write-Verbose "Creating settings variable"
            $global:WTSettingsPath = "$ENV:Userprofile\AppData\Local\Packages\Microsoft.WindowsTerminalPreview_8wekyb3d8bbwe\LocalState\settings.json"
